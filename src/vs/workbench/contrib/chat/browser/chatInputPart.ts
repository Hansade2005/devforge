...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	private readonly urlDetectionRegex = /https?:\/\/[^\s<>"]+/g;
	private detectedUrls: Map<string, { content?: string; state: 'untagged' | 'pending' | 'fetched' | 'error' }> = new Map();
	...
	private async fetchUrlContent(url: string): Promise<void> {
		try {
			const content = await this.sharedWebExtracterService.extract({ uri: URI.parse(url) }, CancellationToken.None);
			this.detectedUrls.set(url, { content: content?.content, state: 'fetched' });
		} catch (err) {
			this.detectedUrls.set(url, { state: 'error' });
		}
		this.renderUrlTags();
	}
	...
	private renderUrlTags() {
		const urlTagsContainer = (this.container.querySelector('.chat-url-tags') as HTMLElement) ||
			(dom.append(this.attachmentsContainer, dom.$('.chat-url-tags')) as HTMLElement);
		dom.clearNode(urlTagsContainer);

		for (const [url, data] of this.detectedUrls) {
			const urlTagElement = dom.$('.chat-url-tag');
			const tag = dom.append(urlTagsContainer, urlTagElement) as HTMLElement;
			const urlText = url.length > 30 ? url.slice(0, 27) + '...' : url;

			let statusIcon = '';
			switch (data.state) {
				case 'pending': statusIcon = '↻'; break;
				case 'fetched': statusIcon = '✓'; break;
				case 'error': statusIcon = '!'; break;
				case 'untagged': statusIcon = '×'; break;
			}

			tag.textContent = `${urlText} ${statusIcon}`;
			...
			tag.addEventListener('click', () => {
				if (data.state === 'fetched') {
					this.detectedUrls.set(url, { ...data, state: 'untagged' });
				} else if (data.state === 'untagged') {
					this.detectedUrls.set(url, { ...data, state: 'fetched' });
				}
				this.renderUrlTags();
			});
		}
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this._register(this._inputEditor.onDidChangeModelContent(() => {
			...
			const matches = inputText.matchAll(this.urlDetectionRegex);
			...
			for (const match of matches) {
				...
				if (!this.detectedUrls.has(url)) {
					this.detectedUrls.set(url, { state: 'pending' });
					this.fetchUrlContent(url);
				}
			}

			// Remove URLs that are no longer in the input
			for (const [url] of this.detectedUrls) {
		...
		// Add after attachmentsToolbarContainer is defined in render()
		const imageInput = document.createElement('input');
		imageInput.type = 'file';
		imageInput.accept = '.png,.jpg,.jpeg,.gif,.bmp,.webp';
		imageInput.style.display = 'none';
		imageInput.multiple = true;
		attachmentToolbarContainer.appendChild(imageInput);

		const imageButton = document.createElement('button');
		imageButton.type = 'button';
		imageButton.className = 'chat-image-upload-button';
		imageButton.title = 'Upload Image';
		imageButton.innerHTML = '<span style="font-size:16px;">🖼️</span>';
		attachmentToolbarContainer.appendChild(imageButton);
		...
		imageInput.addEventListener('change', async (event) => {
			...
			for (const file of files) {
				...
				// Create a fake URI for the image (in-memory)
				const uri = URI.parse(`inmemory://image-upload/${encodeURIComponent(file.name)}`);
				// Add as image attachment
				try {
					await this._attachmentModel.addContext({
						id: uri.toString(),
						name: file.name,
						value: arrayBuffer,
						isImage: true,
