	...
	constructor(
		...
		this.validateParams = {
			...
			web_search: (params: RawToolParamsObj) => {
				const { search_term: searchTermUnknown } = params;
				const search_term = validateStr('search_term', searchTermUnknown);
				return { search_term };
			},
		...
		this.callTool = {
			...
			web_search: async ({ search_term }) => {
				const searchResults = await this.webSearchService.search(search_term);
				const results = searchResults.split('Source: ').filter(Boolean).map(chunk => {
			...
			run_linter: async ({ target }) => {
				...
				return new Promise((resolve, reject) => {
					...
					type ExecError = { code: number; message: string; };

					exec(`npx eslint "${target}" -f json`, (err: ExecError | null, stdout: string, stderr: string) => {
						if (err && !stdout) return reject(new Error(stderr));
						try {
							const results = JSON.parse(stdout);
							resolve({ result: { lintResults: results } });
						} catch {
		...
		this.stringOfResult = {
			...
			web_search: (params, result) => {
				...
				return result.results.map((item, index) => {
					return `Result ${index + 1}: ${item.url}\n${item.snippet}`;
				}).join('\n\n');
			},
			// ---
			...
			list_symbols_in_file: (params, result) => {
				return result.result.map((s: any) => ({
					name: s.name,
					kind: s.kind,
					line: s.line,
